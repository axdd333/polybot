...
pub fn update_underlying_state(
...
pub fn trim_trade_tape(tape: &mut VecDeque<TradeObservation>, now: Instant) {
...
pub fn trade_metrics(tape: &VecDeque<TradeObservation>, now: Instant) -> (f64, f64) {
...
pub fn midpoint(book: &OrderBook) -> f64 {
    let bid = crate::market::book::best_bid(book);
    let ask = crate::market::book::best_ask(book);
    if bid > 0.0 && ask > 0.0 {
        (bid + ask) / 2.0
    } else {
        0.0
